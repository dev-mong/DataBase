...
select ename, job
from em
where sal >  (select sal from emp where empno=7499) --단일행 서브 쿼리 
;
...
from emp
group by job
having avg(sal) <= all(select avg(sal) from emp group by job) --직급 별 평균 급여 비교 
;
select avg(sal) from emp; --전체 직원 평균 급여 
...
select m.ename
from emp m
where m.empno not in (select distinct e.mgr from emp  e where e.mgr is not null);

-- 50. 부하직원이 있는 사원의 이름을 표시하시오.
select m.ename
from emp m
where m.empno in (select distinct e.mgr from emp  e where e.mgr is not null);
...
from emp e
where e.mgr = (select m.empno from emp m where ename='KING')
;
