...
select e.ename, e.empno,nvl(m.ename, '관리자없음')
...
select e.ename, e.deptno,m.ename as 동일부서사원
from emp e, emp m
where e.ename='SCOTT' and e.deptno=m.deptno and m.ename!='scott'
;

select e.ename, deptno, m.ename as 동일부서사원
...
select e.ename,e.hiredate,m.ename as 동일부서사원, m.hiredate as 동일부서사원입사연도
from emp e inner join emp m
...
select e.ename, e.empno, e.hiredate, m.ename as 관리자이름, m.empno as "관리자번호", m.hiredate as "관리자 입사 연도"
from emp e inner join emp m
